...
BubbleChart = (function() {
  function BubbleChart(data) {
    ...
    this.center = {
      x: this.width / 2,
      multi_y: [0,5 * Math.floor(this.height / 6),
                Math.floor(4 * this.height / 6),
                Math.floor(3 * this.height / 6),
                Math.floor(2 * this.height / 6),
                Math.floor(1 * this.height / 6)
               ],
      y: this.height / 2
    };
    this.year_centers = {
    ...
    this.radius_scale = d3.scale.pow().exponent(2).domain([min_amount, max_amount]).range([25, 55]);
    ...
    this.data.forEach((function(_this) {
      return function(d) {
        ...
        node = {
          ...
          year: d.fiscal_year,
          x: _this.year_centers[d.fiscal_year].x,
          y: _this.center.multi_y[d.award_range],
  ...
  BubbleChart.prototype.create_vis = function() {
    ...
    this.texts = this.vis.selectAll("text").data(this.nodes, function(d) {
      return d.id;
    });
    ...
    this.texts.enter().append("text")
        .text((function(_this) {
      return function(d) {
        var txt = addCommas(d.value);
        return txt;
      };
    })(this))
        .attr("dx", (function (_this) {
          return function(d) {
            return _this.year_centers[d.year].x-5;
          }
        })(this))
        .attr("dy", (function (_this) {
          return function(d) {
            return _this.center.multi_y[d.award_range]+5;
          }
        })(this));

    return this.circles.transition().duration(20).attr("r", function(d) {
      return d.radius;
    });
  };
  ...
  BubbleChart.prototype.display_ranges = function() {
    ...
    ranges_y = {
      "> $10,000,000": this.center.multi_y[5]  + offset,
      "< $10,000,000": this.center.multi_y[4]  + offset,
      "< $1,000,000": this.center.multi_y[3]  + offset ,
      "< $100,000": this.center.multi_y[2]  + offset,
      "< $10,000": this.center.multi_y[1] + offset
    };
...
$(function() {
  ...
  return d3.tsv("data/fy11_to_fy15_by_range.csv", render_vis);
});
